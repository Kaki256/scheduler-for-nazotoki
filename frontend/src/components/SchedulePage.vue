<template>
  <div class="schedule-page">
    ...
    <div
      v-if="!loading && initialLoadDone && Object.keys(schedule).length > 0"
      class="schedule-display-section"
    >
      ...
      <div class="calendar-view-container">
              ...
              <tr>
                <th
                  ...
                  :class="{
                    ...
                    'active-header':
                      modalMode === 'weekdayBulk' &&
                      selectedWeekdayForBulkModal === dayIndex &&
                      isModalOpen,
                  }"
                  :style="getWeekdayHeaderStyle(dayIndex)"
                  :aria-pressed="
                    getRepresentativeSlotsForDay(dayIndex).length > 0 &&
                    modalMode === 'weekdayBulk' &&
                    selectedWeekdayForBulkModal === dayIndex &&
                    isModalOpen
                      ? 'true'
            ...
            <tbody>
              <tr v-for="(week, weekIndex) in monthData.weeks" :key="weekIndex">
                <td
                  ...
                  :aria-label="
                    day.isClickable
                      ? `${day.dateString} のスロットを表示`
                      : day.empty
                        ? '空セル'
                        : `${day.dateString} は選択不可`
                  "
                ...
                >
                  <div v-if="!day.empty" class="day-number" :style="{ fontSize: '1em' }">
                    {{ day.date }}
                  </div>
                </td>
              </tr>
        ...
        <p v-if="calendarMonths.length === 0 && initialLoadDone && !loading">
          表示できるカレンダーデータがありません。イベントの募集期間を確認してください。
        </p>
      </div>
      ...
      <div v-if="isModalOpen" class="modal-overlay" @click.self="closeModal">
        <div class="modal-content" role="dialog" aria-modal="true" :aria-labelledby="modalTitleId">
          <button class="modal-close-button" aria-label="閉じる" @click="closeModal">
            &times;
          </button>
          ...
          <div v-if="modalMode === 'date'" class="date-modal-layout">
            <div class="event-slots-container">
              <div class="date-bulk-actions modal-bulk-actions">
                <button
                  :class="[
                    ...
                    'bulk-action-button',
                    'bulk-going',
                    { active: dateActiveBulkStatus[selectedDateForModal] === 'going' },
                  ]"
                ...
                <button
                  :class="[
                    ...
                    'bulk-action-button',
                    'bulk-maybe',
                    { active: dateActiveBulkStatus[selectedDateForModal] === 'maybe' },
                  ]"
                ...
                <button
                  :class="[
                    ...
                    'bulk-action-button',
                    'bulk-not-going',
                    { active: dateActiveBulkStatus[selectedDateForModal] === 'not_going' },
                  ]"
              ...
              <ul v-if="slotsForModal.length > 0" class="slot-list modal-slot-list">
                <li
                  v-for="slot in slotsForModal"
                  :key="slot.originalStartTimeUTC"
                  class="slot-item"
                >
                  <div class="slot-info">
                    <span class="slot-time">{{
                      formatSlotTimeRange(slot.originalStartTimeUTC, parsedEstimatedTimeMinutes)
                    }}</span>
                    <span
                      v-if="slot.status"
                      :class="['slot-availability', getSlotStatusClass(slot.status)]"
                      >{{ formatSlotStatus(slot.status) }}</span
                    >
                  </div>
                  <div
            ...
            <div class="personal-calendar-container">
              ...
              <ul v-if="personalScheduleForSelectedDate.length > 0" class="personal-schedule-list">
                <li
                  v-for="event in personalScheduleForSelectedDate"
                  :key="event.id"
                  class="personal-schedule-item"
                >
                  <strong class="personal-schedule-title">{{ event.title }}</strong>
                  <span class="personal-schedule-time"
                    >{{ formatPersonalEventTime(event.start_datetime) }} -
                    {{ formatPersonalEventTime(event.end_datetime) }}</span
                  >
                </li>
              </ul>
          ...
          <div v-if="modalMode === 'weekdayBulk'">
            <div
              v-if="
                selectedWeekdayForBulkModal !== null &&
                getRepresentativeSlotsForDay(selectedWeekdayForBulkModal).length > 0
              "
              class="weekday-bulk-details-modal"
            >
              <div class="weekday-overall-bulk-actions modal-bulk-actions">
                <button
                  :class="[
                    ...
                    {
                      active:
                        weekdayOverallActiveBulkStatus[selectedWeekdayForBulkModal] === 'going',
                    },
                  ]"
                ...
                <button
                  :class="[
                    ...
                    {
                      active:
                        weekdayOverallActiveBulkStatus[selectedWeekdayForBulkModal] === 'maybe',
                    },
                  ]"
                ...
                <button
                  :class="[
                    ...
                    {
                      active:
                        weekdayOverallActiveBulkStatus[selectedWeekdayForBulkModal] === 'not_going',
                    },
                  ]"
              ...
              <ul class="slot-list modal-slot-list">
                <li
                  v-for="slotRep in getRepresentativeSlotsForDay(selectedWeekdayForBulkModal)"
                  :key="slotRep.originalStartTimeUTC"
                  class="slot-item"
                >
                  <div class="slot-info">
                    <span class="slot-time">{{
                      formatSlotTimeRange(slotRep.originalStartTimeUTC, parsedEstimatedTimeMinutes)
                    }}</span>
                  </div>
                  <div
                    class="user-status-selector clickable"
                    ...
                    @keydown.enter="
                      toggleBulkSlotStatus(selectedWeekdayForBulkModal, slotRep.timeHHMM)
                    "
                    @keydown.space.prevent="
                      toggleBulkSlotStatus(selectedWeekdayForBulkModal, slotRep.timeHHMM)
                    "
            ...
            <p
              v-else-if="selectedWeekdayForBulkModal !== null"
              class="no-representative-slots-message modal-no-events"
            >
              この曜日の代表的なスロットパターンが見つかりません。
            </p>
    ...
    <div
      v-else-if="!loading && initialLoadDone && Object.keys(schedule).length === 0 && !errorMessage"
    ...
    <div v-if="Object.keys(schedule).length > 0 && !isModalOpen" class="save-button-container">
      ...
      <p
        ...
        :class="[
          'save-message',
          saveMessage.type === 'success' ? 'save-message-success' : 'save-message-error',
        ]"
      >
        {{ saveMessage.text }}
      </p>
...
const personalScheduleForSelectedDate = computed(() => {
  ...
  return allPersonalSchedules.value
    .filter((event) => {
...
function formatSlotTimeRange(startTimeUTC, durationMinutes) {
  ...
  try {
    ...
    const startTimeFormatted = startDate.toLocaleTimeString('ja-JP', {
      hour: '2-digit',
      minute: '2-digit',
      timeZone: 'Asia/Tokyo',
    });
    ...
    const endTimeFormatted = endDate.toLocaleTimeString('ja-JP', {
      hour: '2-digit',
      minute: '2-digit',
      timeZone: 'Asia/Tokyo',
    });
    ...
    try {
      return new Date(startTimeUTC).toLocaleTimeString('ja-JP', {
        hour: '2-digit',
        minute: '2-digit',
        timeZone: 'Asia/Tokyo',
      });
    ...
    if (!foundWeekdays.has(dayIndex)) {
      ...
      if (slotsForDate && slotsForDate.length > 0) {
        result[dayIndex] = slotsForDate
          .map((slot) => ({
            timeHHMM: new Date(slot.originalStartTimeUTC).toLocaleTimeString('ja-JP', {
              hour: '2-digit',
              minute: '2-digit',
              timeZone: 'Asia/Tokyo',
            }),
...
const modalTitleComputed = computed(() => {
  if (modalMode.value === 'date' && selectedDateForModal.value) {
    ...
    if (
      parsedEstimatedTimeMinutes.value > 0 &&
      slotsForModal.value &&
      slotsForModal.value.length > 0 &&
      slotsForModal.value[0].originalStartTimeUTC
    ) {
...
function getSlotStatusClass(status) {
  switch (status) {
    ...
    case 'FEW':
      return 'status-few-tickets-left';
    case 'FULL':
      return 'status-sold-out';
    case 'NOT_IN_SALES_PERIOD':
      return 'status-on-sale-soon';
  }
}
...
function setOverallBulkStatusForWeekday(dayIndex, status) {
  ...
  representativeSlots.forEach((slotTimeHHMM) => {
    if (Object.prototype.hasOwnProperty.call(bulkWeekdaySlotSelections[dayIndex], slotTimeHHMM)) {
...
function toggleBulkSlotStatus(dayIndex, slotTimeHHMM) {
  ...
  if (
    weekdayOverallActiveBulkStatus[dayIndex] !== undefined &&
    weekdayOverallActiveBulkStatus[dayIndex] !== null
  )
    weekdayOverallActiveBulkStatus[dayIndex] = null;
  saveMessage.value = { text: '', type: '' };
      ...
      if (schedule.value[dateKey] && Array.isArray(schedule.value[dateKey])) {
        schedule.value[dateKey].forEach((slot) => {
          if (slot && slot.originalStartTimeUTC) {
            let slotTimeHHMM;
            try {
              slotTimeHHMM = new Date(slot.originalStartTimeUTC).toLocaleTimeString('ja-JP', {
                hour: '2-digit',
                minute: '2-digit',
                timeZone: 'Asia/Tokyo',
              });
            ...
            if (Object.prototype.hasOwnProperty.call(selectionsForWeekday, slotTimeHHMM)) {
              ...
              if (
                dateActiveBulkStatus[dateKey] !== undefined &&
  ...
  try {
        ...
        response.data.dates.forEach((dateEntry) => {
          if (dateEntry && dateEntry.date && Array.isArray(dateEntry.slots)) {
            ...
            formattedSchedule[dateKey] = dateEntry.slots.map((slot) => ({
              time: new Date(slot.startAt).toLocaleTimeString('ja-JP', {
                hour: '2-digit',
                minute: '2-digit',
                timeZone: 'Asia/Tokyo',
              }),
    ...
    if (err.response)
      errorMsg +=
        ...
        (err.response.data && err.response.data.error ? `: ${err.response.data.error}` : '');
...
async function fetchEventDetailsBySlugs(orgSlug, eventSlug) {
  ...
  try {
    ...
    if (
      ...
      (locationUidRef.value || orgSlug === 'Yodaka')
    ) {
...
async function loadUserStatus(user, url) {
  ...
  try {
    ...
    if (
      ...
      error.response.data.includes('ReferenceError: rows is not defined')
    ) {
      userFriendlyMessage += ' サーバー内部でエラーが発生しました (データ参照エラー)。';
    } else if (
      error.response &&
      typeof error.response.data === 'string' &&
      error.response.data.toLowerCase().includes('syntaxerror')
    ) {
...
async function saveSelections() {
  ...
  try {
    const selectionsToSave = Object.keys(userSelection).map((key) => ({
      event_datetime_utc: key,
      status: userSelection[key],
    }));
    const response = await axios.post(`${API_BASE_URL}/save-my-status`, {
      username: username.value,
      eventUrl: eventUrlRef.value,
    ...
    if (response.status === 204) {
      ...
      saveMessage.value = {
        text: '選択の保存に失敗しました。サーバーからの応答が予期せぬものです。',
        type: 'error',
      };
    }
  } catch (error) {
...
const slotsForModal = computed(() => {
  if (
    ...
    !schedule.value[selectedDateForModal.value]
  )
    return [];
...
const calendarMonths = computed(() => {
  ...
  while (
    ...
    (currentDateIterator.getUTCFullYear() === loopEndDate.getUTCFullYear() &&
      currentDateIterator.getUTCMonth() <= loopEndDate.getUTCMonth())
  ) {
...
const getWeekdayHeaderStyle = (dayIndex) => {
  ...
  representativeSlots.forEach((slotTimeHHMM, index) => {
    const status =
      bulkWeekdaySlotSelections[dayIndex] && bulkWeekdaySlotSelections[dayIndex][slotTimeHHMM];
...
.personal-schedule-time {
  font-size: 0.9em;
  color: #495057;
}
.personal-calendar-container .no-events-message {
  font-size: 0.9em;
  color: #6c757d;
  padding-top: 10px;
}
