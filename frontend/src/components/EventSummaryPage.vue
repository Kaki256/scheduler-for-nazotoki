  ...
  <div class="container p-3">
    <header class="mb-4">
      ...
      <p v-if="eventStartDate && eventEndDate" class="text-gray-600 text-sm mt-1">
        対象期間: {{ formatDateForDisplay(eventStartDate) }} 〜
        {{ formatDateForDisplay(eventEndDate) }}
      </p>
    ...
    <div
      v-if="!loadingInitialData && !errorMessage && groupedTimeSlotsForTable.length > 0"
      class="summary-table-container mb-6"
    >
      <!-- margin-bottom reduced -->
      <h2 class="text-xl font-semibold text-gray-800 mb-2">出欠詳細</h2>
      <!-- margin-bottom reduced -->
      ...
      <table class="summary-table">
        ...
        <tbody>
          <template v-for="group in groupedTimeSlotsForTable" :key="group.dateKey">
            <tr v-for="slot in group.slots" :key="group.dateKey + '-' + slot.fullUtc">
    ...
    <!-- End of summary-table-container -->

    <!-- Fixed Teams Configuration Section Wrapper -->
    <div v-if="!loadingInitialData && !errorMessage && participatingUsers.length > 0" class="mb-6">
      <button class="button-secondary mb-2" @click="toggleFixedTeamsSection">
        {{ isFixedTeamsSectionVisible ? '固定チーム設定を隠す' : '固定チーム設定を表示' }}
      </button>

      <div
        v-if="isFixedTeamsSectionVisible"
        class="fixed-teams-configurator p-4 border rounded-lg shadow-md bg-gray-100"
      >
            ...
            >
              ...
              <p
                v-if="
                  ...
                  fixedTeams.flat().length === participatingUsers.length
                "
                class="text-sm text-gray-500"
              >
                全参加者が固定チームに割当済です。
              </p>
              <p v-else-if="availableUsersForDnd.length === 0" class="text-sm text-gray-500">
                固定できる参加者がいません。
              </p>
            </div>
          </div>
            ...
            >
              ...
              >
                <div class="flex justify-between items-center mb-1">
                  <h4 class="font-semibold text-gray-800">チーム {{ teamIndex + 1 }}</h4>
                  <!-- Changed text-indigo-600 to text-gray-800 -->
                  <button
                    class="text-red-500 hover:text-red-700 text-xs"
                    @click="removeFixedTeam(teamIndex)"
                  >
                    チーム削除
                  </button>
                </div>
                <div
                  class="min-h-[30px] border border-dashed border-indigo-300 p-1 rounded drop-target-inner-team"
                >
                  <span
                  ...
                  <p v-if="team.length === 0" class="text-xs text-gray-400 italic">
                    このチームにメンバーをドラッグ
                  </p>
                </div>
              </div>
              <p v-if="fixedTeams.length === 0" class="text-sm text-gray-500 p-2">
                ここに右のリストからユーザーをドラッグして新しい固定チームを作成するか、下のボタンで空のチームを追加できます。
              </p>
              <button class="mt-2 button-small-secondary" @click="addNewFixedTeam">
                新しい空の固定チームを追加
              </button>
            </div>
          </div>
        </div>
        <button class="mt-4 button-primary" @click="applyFixedTeamsAndRegenerate">
          固定チームを適用して再計算
        </button>
        <button class="mt-4 ml-2 button-secondary" @click="resetFixedTeams">固定をリセット</button>
      </div>
    </div>
    ...
    <div
      v-if="!loadingTeamCombinations && !errorMessage && sortedTeamCombinations.length > 0"
      class="team-combinations-container mt-6"
    >
      <!-- margin-top reduced -->
      <h2 class="text-xl font-semibold text-gray-800 mb-2">チーム編成案 (総合スコア順)</h2>
      <!-- margin-bottom reduced -->

      <!-- Pagination Controls -->
      <div
        v-if="totalPages > 1"
        class="pagination-controls my-3 flex justify-center items-center space-x-2"
      >
        <button
      ...
      <p v-if="sortedTeamCombinations.length > 0" class="text-sm text-gray-600 text-center mb-3">
        全 {{ sortedTeamCombinations.length }} 件中 {{ (currentPage - 1) * itemsPerPage + 1 }} -
        {{ Math.min(currentPage * itemsPerPage, sortedTeamCombinations.length) }} 件表示
      </p>

      <div
        v-for="(combination, comboIndex) in paginatedTeamCombinations"
        :key="comboIndex"
        class="team-combination-card mb-4 p-3 border rounded-lg shadow-md bg-gray-50"
      >
        <!-- padding and margin reduced -->
        <h3 class="text-lg font-bold text-gray-900 mb-2">
          <!-- text-green-700 removed, text-gray-900 added, margin-bottom reduced -->
          編成案 {{ (currentPage - 1) * itemsPerPage + comboIndex + 1 }}
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          <!-- gap reduced -->
          <div
            v-for="(team, teamIndex) in combination.structureDetails"
            :key="teamIndex"
            class="team-card bg-white p-2 rounded-lg shadow"
          >
            <!-- padding reduced -->
            ...
            <ul class="list-disc list-inside pl-3 text-sm text-gray-600 mt-1">
              <!-- padding-left reduced, margin-top added -->
              <li v-for="member in team.members" :key="member" class="my-0.5">
                <span
                  v-if="team.bestSlotsDetails.length > 0 && userSelectionsMap[member]"
                  :class="[
                    'ml-1 text-xs font-mono px-1 rounded',
                    getStatusClass(userSelectionsMap[member]?.[team.bestSlotsDetails[0].utc]),
                  ]"
                ...
                <span
                  v-else-if="team.bestSlotsDetails.length > 0"
                  class="ml-1 text-xs text-gray-400 font-mono px-1 rounded status-unknown"
                  :title="`Status for ${team.bestSlotsDetails[0].dateLabel} ${team.bestSlotsDetails[0].timeLabel}`"
                  >(-)</span
                >
                {{ member }}
            ...
            <div v-if="team.bestSlotsDetails && team.bestSlotsDetails.length > 0">
              ...
              <ul class="text-xs text-gray-600 list-none pl-1">
                ...
                >
                  <span
                    v-if="vacancyStatusMap[slotDetail.utc]"
                    :class="getVacancyStatusClass(vacancyStatusMap[slotDetail.utc])"
                    class="mr-1 vacancy-indicator w-6 text-center inline-block"
                  >
                    {{ formatVacancyStatus(vacancyStatusMap[slotDetail.utc]) }}
                  </span>
                  <span
                    v-else
                    class="mr-1 vacancy-indicator text-gray-400 w-6 text-center inline-block"
                    >?</span
                  >
    ...
    <div
      v-if="
        !loadingInitialData &&
        !loadingTeamCombinations &&
        !errorMessage &&
        sortedTeamCombinations.length === 0 &&
        participatingUsers.length > 0 &&
        allEventTimeSlotsUTC.length > 0 &&
        maxParticipants > 0
      "
      class="no-data-message"
    >
      <p class="text-gray-600">
        条件に合うチーム編成案を生成できませんでした。参加人数やチームの最大人数設定を確認してください。
      </p>
    </div>
    <div
      v-if="
    ...
    <div
      v-if="
        !loadingInitialData &&
        !errorMessage &&
        groupedTimeSlotsForTable.length === 0 &&
        allEventTimeSlotsUTC.length > 0
      "
      class="no-data-message"
    >
      <p class="text-gray-600">
        集計可能な日時スロットがありませんでした。イベント期間を確認してください。
      </p>
    </div>
    <div
      v-if="!loadingInitialData && !errorMessage && allEventTimeSlotsUTC.length === 0"
      class="no-data-message"
    >
  ...
  try {
    ...
    if (props.orgSlug === 'Yodaka') {
      try {
        ...
        errorMessage.value =
          'Yodakaイベント詳細の取得中にエラーが発生し、売れ行き状況を表示できません。';
      }
    }
      ...
      try {
        ...
        if (eventDetailsResponse.ok) {
          ...
          if (details.locationUid) {
            ...
            // エラーメッセージに追記する形に変更
            const msg =
              'イベントの場所情報(locationUid)の取得に失敗しました。売れ行き状況を表示できません。';
    ...
    if (
      ...
      allEventTimeSlotsUTC.value.length > 0 &&
...
async function fetchVacancyData() {
  ...
  const reconstructedEventUrl =
    props.orgSlug === 'Yodaka'
      ? `https://yodaka.info/event/${props.eventSlug}/`
      : `https://escape.id/${props.orgSlug}-org/e-${props.eventSlug}/`;

  console.log(
    'Fetching vacancy data for:',
...
function formatDateTimeForHeader(utcDateTimeString) {
  // This is used in getStatusTitle, keep as is or make specific if needed
...
function handleDropOnFixedTeamsArea(event, targetTeamIndex) {
  ...
  if (
    ...
    fixedTeams.value[targetTeamIndex].length >= maxParticipants.value &&
    !fixedTeams.value[targetTeamIndex].includes(user)
  ) {
  ...
  if (
    fromTeamIndex !== null &&
    typeof fromTeamIndex === 'number' &&
    fixedTeams.value[fromTeamIndex]
  ) {
    // Dragged from an existing fixed team
    fixedTeams.value[fromTeamIndex].splice(memberIndexInTeam, 1);
  }

  // 2. Add to new position
  if (targetTeamIndex === -1) {
    // Dropped on the general "Fixed Teams Area" to create a new team
    ...
    // Dropped on an existing team card (targetTeamIndex)
    if (!fixedTeams.value[targetTeamIndex]) {
      fixedTeams.value[targetTeamIndex] = [];
    }
    if (!fixedTeams.value[targetTeamIndex].includes(user)) {
...
.summary-table-container {
  ...
  box-shadow:
    ...
    0 1px 2px -1px rgba(0, 0, 0, 0.05); /* Adjusted shadow */
}
...
.slot-time {
  /* This is duplicated, ensure correct one is used or merge */
  margin-right: 0.4em;
  color: #4b5563;
  white-space: nowrap;
}
...
.button-page-number.bg-indigo-500 {
  /* Active page style */
...
.drop-target-inner-team {
  /* Use this class for the inner droppable area of a team card */
  min-height: 30px;
  padding: 0.25rem;
...
.sticky-col.header-dategroup,
.sticky-col.header-username {
